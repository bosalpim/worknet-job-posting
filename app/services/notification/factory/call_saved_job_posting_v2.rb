class Notification::Factory::CallSavedJobPostingV2 < Notification::Factory::NotificationFactoryClass
    ...
    @list.each do |saved_job_posting|
      ...
      if @target_medium == APP_PUSH
        if user.is_sendable_app_push
          ...
          create_bizm_post_pay_message(customer, job_posting, user)
        end
      else
        create_bizm_post_pay_message(customer, job_posting, user)
      end
  ...
  def create_app_push_messsage(job_posting, user)
  ...
  def create_bizm_post_pay_message(customer, job_posting, user)
    # 메시지 데이터 > 어르신 정보
