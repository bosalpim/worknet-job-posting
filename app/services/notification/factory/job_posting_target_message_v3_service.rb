class Notification::Factory::JobPostingTargetMessageV3Service < Notification::Factory::NotificationFactoryClass
  ...
  def initialize(params)
    super(MessageTemplateName::TARGET_USER_JOB_POSTING_V3)
    @params = params
  ...
  def create_bizm_post_pay_message(user, dispatched_notification_param, application_notification_param)
    ...
    link = "#{base_url}/jobs/#{@job_posting.public_id}?referral=target_notification&utm_source=message&utm_medium=arlimtalk&utm_campaign=#{@message_template_id}&lat=#{user.lat}&lng=#{user.lng}" + dispatched_notification_param
    mute_link = "#{base_url}/me/notification/off?type=job&utm_source=message&utm_medium=arlimtalk&utm_campaign=#{@message_template_id}"
