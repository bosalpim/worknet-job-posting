class JobSupportProject::SecondSubmitRemindService
  def initialize
  ...
  def call
    ...
    @job_support_project_participants.each do |job_support_project_participant|
      ...
      Jets.logger.info "#{job_support_project_participant.id} 대상 2차 리마인드 발송\n"
      ...
      today_date = (@kst_now + 1.days).in_time_zone('Asia/Seoul').strftime('%m.%d')

      if business_registration.nil?
        Jets.logger.info "사업자 등록증 없는 경우"
        message = "안녕하세요 케어파트너입니다.
채용지원금 신청 서류 제출 일자가 지나 다시 연락드렸습니다.
