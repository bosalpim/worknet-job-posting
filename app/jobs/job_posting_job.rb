  ...
  def first_message
    ...
    if Jets.env != 'production'
      begin
        ...
        second_reserve_notification = Notification::FactoryService.create(MessageTemplateName::JOB_ADS_MESSAGE_RESERVE, { job_posting_id: job_posting_id, times: 2, scheduled_at_text: (scheduled_at).strftime('%m월 %d일 %I시 %M분') })
        ...
        third_reserve_notification = Notification::FactoryService.create(MessageTemplateName::JOB_ADS_MESSAGE_RESERVE, { job_posting_id: job_posting_id, times: 3, scheduled_at_text: (scheduled_at).strftime('%m월 %d일 %I시 %M분') })
